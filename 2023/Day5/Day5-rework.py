    ...
    with open(file, 'r') as text:
        ...
        with Pool(processes=15) as pool:
            args = [(task_id, item,
                     ...
                     temp_to_humid_map,
                     humid_to_loc_map)
                    for task_id, item in enumerate(chunk(part_2_seeds, chunk_size=10_000_000))]
            total_tasks = len(args)
            print(f"{total_tasks} tasks to compute")

            part_2_locations = 1000000000000
            tasks_done = 0
            for task_id, potential_min in pool.imap_unordered(find_min_seed, args):
                tasks_done += 1
                print(f"Got result from a task #{task_id}: {potential_min}, {tasks_done}/{total_tasks} done.")
...
def find_min_seed(params):
    task_id, item, seeds_to_soil_map, soil_to_fert_map, fert_to_water_map, water_to_light_map, light_to_temp_map, temp_to_humid_map, humid_to_loc_map = params
